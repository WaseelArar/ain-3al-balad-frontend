<script lang="ts">
    ...
    import { ThumbsUp, Volume2 } from "lucide-svelte";
    ...
    async function handleDeleteIssue() {
        if (!issuesId || deletingIssue) return;
        if (!confirm("هل أنت متأكد من حذف هذا البلاغ؟")) return;

        deletingIssue = true;
        try {
            await api.delete(`/issues/${issuesId}`);
            await issueStore.refresh();
            goto("/issue/feed");
        } catch (err) {
                    ...
                    {#if $user?.role === "ADMIN" || $user?.role === "MUNICIPAL"}
                        <div class="flex flex-wrap gap-2">
                            ...
                            <Button
                                ...
                                disabled={deletingIssue}
                                class="gap-2 rounded-full border-red-200 text-red-600 hover:bg-red-50 hover:text-red-700"
                            >
                                <Trash2 class="size-4" />
                                {deletingIssue ? "جاري الحذف..." : "حذف البلاغ"}
                            </Button>
                        </div>
                    {:else}
                        <div
                ...
                {#if issue.audio}
                    <div class="mb-6 p-4 bg-slate-50 border border-slate-200 rounded-2xl">
                        <div class="flex items-center gap-2 mb-3">
                            ...
                            <p class="text-sm font-bold text-slate-700">تسجيل صوتي مرفق</p>
                        </div>
                        <audio controls src={baseUrl + issue.audio} class="w-full rounded-xl" preload="metadata">
