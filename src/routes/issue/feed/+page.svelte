<script lang="ts">
    ...
    import api, { baseUrl } from "$lib/api/api";
    ...
    const MIN_ISSUE_DATE = "2026-05-01";
    ...
    let deletingIssueIds = $state<number[]>([]);
    let maxIssueDate = $state(getTodayInputValue());
    ...
    onMount(() => {
        ...
        const interval = setInterval(() => {
            maxIssueDate = getTodayInputValue();
        }, 60000);
    ...
    $effect(() => {
        if ($user?.role !== "ADMIN" && $user?.role !== "MUNICIPAL" && $issueStore.issueDate) {
            issueStore.setIssueDate(undefined);
        }
    });
    ...
    function handleDateFilter(e: Event) {
        const target = e.target as HTMLInputElement;
        issueStore.setIssueDate(target.value || undefined);
    }
    ...
    async function handleDeleteIssue(issueId: number, event: MouseEvent) {
        event.preventDefault();
        event.stopPropagation();

        if (deletingIssueIds.includes(issueId)) return;
        if (!confirm("هل أنت متأكد من حذف هذا البلاغ؟")) return;

        deletingIssueIds = [...deletingIssueIds, issueId];
        try {
            ...
            deletingIssueIds = deletingIssueIds.filter((id) => id !== issueId);
        }
    }
</script>
        ...
        <div class="relative w-full md:w-72">
            ...
            {#if $user?.role === "ADMIN" || $user?.role === "MUNICIPAL"}
                <div class="flex w-full gap-2 md:w-auto">
                    <div class="relative flex-1 md:w-42">
                        <Calendar
                            class="absolute right-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground"
                        />
                        <Input
                            ...
                            min={MIN_ISSUE_DATE}
                            max={maxIssueDate}
                            value={$issueStore.issueDate ?? ""}
                            onchange={handleDateFilter}
                            class="pr-10"
                        />
                    </div>
                    {#if $issueStore.issueDate}
                        <Button
                            variant="outline"
                            size="sm"
                            onclick={() => issueStore.setIssueDate(undefined)}
                            class="shrink-0"
                        >
                            كل التواريخ
                        </Button>
