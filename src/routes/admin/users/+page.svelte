<script lang="ts">
    ...
    import { user as currentUser } from "@/stores/auth.store";
    ...
    let canManageUsers = $derived($currentUser?.role === "ADMIN");
...
<div class="p-6 max-w-6xl mx-auto" dir="rtl">
    ...
    >
        <table class="w-full text-right border-collapse {!canManageUsers ? 'hide-actions' : ''}">
                ...
                {#each $adminUsers.items as user}
                    ...
                    >
                        ...
                        <td class="p-4">
                            ...
                            {:else}
                                <div
                                    class="mt-2 inline-flex h-8 items-center rounded-full border border-slate-200 px-3 text-[11px] font-bold {roleConfig[
                                        user?.role ?? 'USER'
                                    ].color}"
                                >
...
{#if canManageUsers && isPasswordModalOpen}
    <div
...
<style>
    .hide-actions :global(th:nth-child(5)),
    .hide-actions :global(td:nth-child(5)) {
        display: none;
    }
</style>
